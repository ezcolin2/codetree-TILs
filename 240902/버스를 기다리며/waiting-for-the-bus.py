...
def is_possible(wait_time):
    group_cnt = 0 # 총 그룹 수
    current_group_cnt = 0 # 현재 그룹 내부 요소 개수
    current_group_start_idx = 0
    for i in range(n):
        # 만약 그룹이 꽉 찼거나 기다리는 시간이 길다면 새로운 그룹 생성
        if group_cnt == 0 or current_group_cnt == c or arr[i]-arr[current_group_start_idx] > wait_time:
            current_group_cnt = 0
            group_cnt += 1
            current_group_start_idx = i
        # 기다리는 시간보다 작으면 현재 그룹에 넣기
        #if arr[i]-arr[current_group_start_idx] <= wait_time:
        current_group_cnt += 1
    # 생성된 그룹의 수가 M 이하인지
    return group_cnt <= m
